...
export default function Page() {
  ...
  const handleSearchRecomendation = async () => {
    ...
    if (descripcionUser.trim()) {
      const response = await fetch(`/api/recomendation-ia/${encodeURIComponent(descripcionUser)}/${gender}`);
      const data = await response.json();
  ...
  const getFilteredProducts = () => {
    ...
    return Object.fromEntries(Object.entries(productsApi).filter(([key]) => activeFilters.includes(key)));
  };
  ...
  const renderFilters = () => (
    <div className="p-4 rounded-lg shadow-md">
      ...
      {productsApi && Object.keys(productsApi).map((key) => (
  ...
  const renderSection = (title: string, products: any[]) => (
    <div key={title} className="mb-8 mt-3 p-3">
      ...
      <div className={view === 'grid' ? 'grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-6' : 'space-y-4'}>
        {products.map((product, index) => view === 'grid' ? <CardGrid key={index} product={product} /> : <CardList key={index} product={product} />)}
      </div>
    </div>
  );
      ...
      <div className="bg-gray-100 py-4 shadow-sm">
        <SortBar view={view} onViewChange={setView}>
          <div className="flex flex-wrap items-center gap-4 px-4">
            ...
            />
            <button hidden className="px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition focus:outline-none">
              Obtener Recomendación
            </button>
            <Autocomplete eventos={Eventos} onSelect={handleSelectEvento} />
            <button onClick={handleSearch} disabled={search.length < 2} className="px-4 py-2 text-white bg-green-500 rounded-lg hover:bg-green-600 transition focus:outline-none">
              Buscar
            </button>
            <button onClick={getRandomRecomendation} hidden={!productsApi} className="px-4 py-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition focus:outline-none">
              No sé qué elegir
            </button>
          ...
          {randomRecomendation.length > 0 && (
            <section className="mt-8">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setRandomRecomendation([])} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                  <Trash />
                </button>
              </div>
              <div className={view === "grid" ? "grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6" : "space-y-4"}>
                {randomRecomendation.map((product: any, index: number) => view === "grid" ? <CardGrid key={index} product={product} /> : <CardList key={index} product={product} />)}
              </div>
            </section>
          )}
